...
const deleteUser = async (req, res, next) => {
  ...
  try {
    ...
    if (Number(id) === 11) {
      const error = new Error('El administrador principal no se puede anonimizar');
    ...
    if (req.userAuth.id !== Number(id) && req.userAuth.role !== 'admin') {
      const error = new Error('No tienes permisos para anonimizar este usuario');
    ...
    await connection.query(
      `
      UPDATE usuarios
      SET contraseña=?, nombre="[borrado]", apellidos="[borrado]", correo="[borrado]",metodoDePago=NULL, fechaNacimiento=NULL, avatar=NULL, active=0, deleted=1, lastAuthUpdate=?
      WHERE idUsuario=?
    `,
